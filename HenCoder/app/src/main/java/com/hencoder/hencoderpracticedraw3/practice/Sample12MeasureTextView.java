package com.hencoder.hencoderpracticedraw3.practice;

import android.content.Context;
import android.graphics.Canvas;
import android.graphics.Paint;
import android.support.annotation.Nullable;
import android.util.AttributeSet;
import android.view.View;

/**
 * 作者:幻海流心
 * GitHub:https://github.com/HuanHaiLiuXin
 * 邮箱:wall0920@163.com
 * 2018/4/19 16:41
 */
public class Sample12MeasureTextView extends View {
    private Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
    private String s1 = "你真是个";
    private String s2 = "傻屌,";
    private String s3 = "切克闹";
    {
        paint.setTextSize(60);
    }

    public Sample12MeasureTextView(Context context) {
        this(context,null,0);
    }
    public Sample12MeasureTextView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs,0);
    }
    public Sample12MeasureTextView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    @Override
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        float w1 = paint.measureText(s1);
        float w2 = paint.measureText(s2);
        float w3 = paint.measureText(s3);
        canvas.drawText(s1,100,100,paint);
        canvas.drawText(s2,100 + w1,100,paint);
        canvas.drawText(s3,100+w1+w2,100,paint);
    }
}
